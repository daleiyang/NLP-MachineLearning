...
> 2. 0.先用结构化平均感知机学习算法学习特征的概率，然后用类似HMM的生成式模型生成结果，这是很先进的方法. 1. 词性标注的特征抽取方法. 2.Collins论文中的算法是如何在结构化平均感知机的学习算法中实现的.
> 3. 0.先用结构化平均感知机学习算法学习特征的概率，然后用类似HMM的生成式模型生成结果，这是很先进的方法. 1. 分词，命名实体识别采用的特征抽取方法、词性标注的特征抽取方法其实大同小异. 2. 如何实现Collins论文中的学习算法、面对L1,L2正则化时如何变通 3. 使用Backoff05的MSR训练、测试语料，经过5轮训练，无正则化处理，F值在 0.9472. 4. 存在的问题是L1,L2的学习率需要调整，模型剪裁也还没实验 5.体会到python用很短的代码就能够完成复杂功能，但是训练起来很慢。6. 很想做一份C的实现，和清华分词比比效果，以后有时间再做吧.
