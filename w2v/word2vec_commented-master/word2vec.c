 ...
 */
int AddWordToVocab(char *word, char *type) {
  // Measure word length.
  unsigned int hash, length = strlen(word) + 1, typeLen = strlen(type) + 1;
  
  // Limit string length (default limit is 100 characters).
  if (length > MAX_STRING) length = MAX_STRING;
  if (typeLen > MAX_STRING) typeLen = MAX_STRING;
  
  // Allocate and store the word string.
  vocab[vocab_size].word = (char *)calloc(length, sizeof(char));
  vocab[vocab_size].type = (char *)calloc(typeLen, sizeof(char));
  strcpy(vocab[vocab_size].word, word);
  strcpy(vocab[vocab_size].type, type);
     ...
     */
      ...
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        if (negative > 0) for (d = 0; d < negative + 1; d++) {
          ...
          if (d == 0) {
            if(srtcmp(vocab[last_word].type, vocab[word].type)) {f = true; break;}
            target = word;
            label = 1;
        ...
        if (!f) for (c = 0; c < layer1_size; c++) syn0[c + l1] += neu1e[c];
      }
    }
